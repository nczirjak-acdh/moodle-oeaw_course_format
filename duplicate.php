...
$PAGE->set_url('/course/format/oeaw_cf/duplicate.php', array('courseid' => $courseid, 'section' => $section));
...
if (!empty($sectioninfo)) {
    ...
    $pbar = new progress_bar('oeaw_cf_duplicate_bar', 500, true);
    $pbar->update_full(1, get_string('duplicating', 'format_oeaw_cf'));
    ...
    $pbar->update_full(5, get_string('creating_section', 'format_oeaw_cf'));
    ...
    $pbar->update_full(10, get_string('rebuild_course_cache', 'format_oeaw_cf'));
    ...
    if (is_object($modinfo) && isset($modinfo->sections[$section])) {
        ...
        if (is_array($section_mods)) {
            ...
            $k = 0;
            $pbar->update_full(40, get_string('progress_counter', 'format_oeaw_cf', $data_progress));
            foreach ($section_mods as $modnumber) {
                ...
                $pbar->update_full($percent, get_string('progress_counter', 'format_oeaw_cf', $data_progress));
            }
        }
    }
    else {
        $pbar->update_full(100, get_string('progress_full', 'format_oeaw_cf'));
    }
    ...
    echo get_string('error_nosectioninfo', 'format_oeaw_cf');
