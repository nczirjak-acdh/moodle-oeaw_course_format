...
/**
 * Basic renderer for oeaw_cf format.
 *
 * @copyright 2012 David Herney Bernal - cirano
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_oeaw_cf_renderer extends format_section_renderer_base {
    ...
    /* 
     * 
     * get the course section name for the menu
     * 
     */
    ...
    public function print_single_section_page($course, $sections, $mods, $modnames, $modnamesused, $displaysection) {
        ...
        //this contains the sections too
        ...
        if ($course->realcoursedisplay == COURSE_DISPLAY_MULTIPAGE) {
            ...
            if ((($thissection->visible && $thissection->available) || $canviewhidden) && 
                ...
                else if ($this->_course->templatetopic == format_oeaw_cf::TEMPLATETOPIC_LIST) {
        ...
        echo html_writer::start_tag('div', array('class' => 'single-section oeaw_cf'));
...
/*
  ...
  */                  
                    ...
                    //start_tag($tagname, array $attributes = null);
                    
                    $courseSections =  html_writer::alist($this->getCourseSectionNames($course->id, $section), array('class' => 'class1'), 'ul');
                    //$courseSections = $this->courserenderer->course_section_cm_list($course, $section);
                    
                    
                    // here generating the tab content
                    /*$new_tab = new tabobject("tab_topic_" . $section, $url,
                    '<div style="' . $custom_styles . '" class="tab_content ' . $special_style . '"><span>' . s($sectionname) ."</span><br>"
                            . "<div style='color:black;background-color:white;'>".$courseSections."</div>"
                            . "</div>");
                    */
                    //echo "<pre>";
                    //var_dump((array)$new_tab->text);
                    //echo "</pre>";
                        ...
                        if($format_options['level'] == 0 || count($tabs) == 0) {                                       
                            ...
                            if (!is_array($tabs[$parent_index]->subtree)) {
                                ...
                                if ($parentformat_options['firsttabtext']) {
                                    ...
                                    $firsttab_text = get_string('index', 'format_oeaw_cf');
                                }
                    ...
                    if ($can_move) {
                        if ($section > 0) { // Move section
                            ...
                            /*
                            $strsubtopictoright = get_string('subtopictoright', 'format_oeaw_cf');
        ...
        echo html_writer::div('', 'menu_section_div_content');
         ...
         ?>
               ...
               /*         $('.menu_course_section_value').click(function() {
                            ...
                            var module = arr[0];
                            var instance = arr[1];
                           //  $(".menu_section_div_content").html('<object data="http://tired.com/">');
                            $(".menu_section_div_content").load("https://clarin.oeaw.ac.at/moodle-dev/mod/" + module + "/view.php?id=" + instance +" #page");
                            console.log("https://clarin.oeaw.ac.at/moodle-dev/mod/" + module + "/view.php?id=" + instance +" #page");
                        });
                    });
*/
                    </script>
            <?php
               ...
               // echo html_writer::div(html_writer::alist($this->getCourseSectionNames($course->id, $displaysection)), 'menu_section_div_content');
        ...
        if (!$sections[$displaysection]->uservisible && !$canviewhidden) {
            ...
            /// topicsot itt generalja le a contentet
            if ($course->realcoursedisplay != COURSE_DISPLAY_MULTIPAGE || $displaysection !== 0) {
               ...
               // echo $this->section_header($thissection, $course, true);
                ...
                if ($this->_course->templatetopic == format_oeaw_cf::TEMPLATETOPIC_NOT) {
                    ...
                    echo $this->custom_course_section_cm_list($course, $thissection);
                }
  ...
  //              echo $this->courserenderer->course_section_add_cm_control($course, $displaysection, $displaysection);
        ...
        if ($PAGE->user_is_editing() && has_capability('moodle/course:update', $context)) {
            ...
            print_collapsible_region_start('move-list-box clearfix collapsible mform', 'course_format_oeaw_cf_config_movesection', get_string('utilities', 'format_oeaw_cf'), '', true);
            ...
            if ($can_move && !empty($move_list_html)) {
                echo html_writer::start_div("form-item clearfix");
                    echo html_writer::start_div("form-label");
                        echo html_writer::tag('label', get_string('movesectionto', 'format_oeaw_cf'));
                    ...
                    echo html_writer::start_div("form-description");
                        echo html_writer::tag('p', get_string('movesectionto_help', 'format_oeaw_cf'));
                    echo html_writer::end_div();
                echo html_writer::end_div();
            }
            ...
            global $USER, $OUTPUT;
            if (isset($USER->oeaw_cf_da[$course->id]) && $USER->oeaw_cf_da[$course->id]) {
                $url->param('oeaw_cf_da', 0);
                $text_button_disableajax = get_string('enable', 'format_oeaw_cf');
            }
            else {
                $url->param('oeaw_cf_da', 1);
                $text_button_disableajax = get_string('disable', 'format_oeaw_cf');
            }

            echo html_writer::start_div("form-item clearfix");
                echo html_writer::start_div("form-label");
                    echo html_writer::tag('label', get_string('disableajax', 'format_oeaw_cf'));
                echo html_writer::end_div();
                echo html_writer::start_div("form-setting");
                    echo html_writer::link($url, $text_button_disableajax);
                echo html_writer::end_div();
                echo html_writer::start_div("form-description");
                    echo html_writer::tag('p', get_string('disableajax_help', 'format_oeaw_cf'));
                echo html_writer::end_div();
            echo html_writer::end_div();

            //Duplicate current section option
            if (has_capability('moodle/course:manageactivities', $context)) {
                $url_duplicate = new moodle_url('/course/format/oeaw_cf/duplicate.php', array('courseid' => $course->id, 'section' => $displaysection, 'sesskey' => sesskey()));

                $link = new action_link($url_duplicate, get_string('duplicate', 'format_oeaw_cf'));
                $link->add_action(new confirm_action(get_string('duplicate_confirm', 'format_oeaw_cf'), null, get_string('duplicate', 'format_oeaw_cf')));

                echo html_writer::start_div("form-item clearfix");
                    echo html_writer::start_div("form-label");
                        echo html_writer::tag('label', get_string('duplicatesection', 'format_oeaw_cf'));
                    echo html_writer::end_div();
                    echo html_writer::start_div("form-setting");
                        echo $this->render($link);
                    echo html_writer::end_div();
                    echo html_writer::start_div("form-description");
                        echo html_writer::tag('p', get_string('duplicatesection_help', 'format_oeaw_cf'));
                    echo html_writer::end_div();
                echo html_writer::end_div();
            }
    ...
    private function replace_resources ($section) {
        ...
        if ($this->_course->templatetopic_icons == 0) {
            $summary = '<span class="oeaw_cf_hideicons">' . $summary . '</span>';
        }
